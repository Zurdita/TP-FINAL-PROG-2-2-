...
package com.ifes.servicio;
import com.ifes.dom.*;
import com.ifes.repositorio.RepositorioConcesionaria;
...
public class ServicioConcesionaria {
    ...
    public void guardar(Concesionaria c) { repo.guardar(c); }
    public List<Concesionaria> listar() { return repo.listar(); }
    public void eliminar(Concesionaria c) { repo.eliminar(c); }

    public String informe() {
        int autos=0, camionetas=0;
        for(Concesionaria c: listar()) {
            for(Rodado r: c.getRodados()) {
                if(r instanceof Auto) autos++;
                else if(r instanceof Camioneta) camionetas++;
            }
        }
...
public void actualizarConcesionaria(Concesionaria c) { guardar(c); }
